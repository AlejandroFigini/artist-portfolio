...
const CONTAINER_BASES: Record<string, { section: string; label: (n: number) => string; kind: 'image' | 'video' | 'text' }> = {
  ...
  'model3d.soft': { section: '3D Models', label: (n) => `Logo Software 3D #${n}`, kind: 'image' },
  'model3d.title': { section: '3D Models', label: (n) => `Título 3D #${n}`, kind: 'text' },
  'model3d.desc': { section: '3D Models', label: (n) => `Texto 3D #${n}`, kind: 'text' },
