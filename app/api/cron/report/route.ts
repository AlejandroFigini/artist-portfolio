...
function buildReportHtml(m: Metrics): string {
  ...
  const body = `
    ...
    <div style="margin-top: 32px;">
      ...
      ${list(m.topCountries.map((c) => `<span style="color: #0f172a; font-weight: 500;">${escapeHtml(c.name)}</span>: ${c.users} active (${c.returningUsers} returning, ${c.newUsers} new)`))}
    </div>
    ` : ''}
    ...
    <div style="margin-top: 32px;">
      ...
      ${list(m.historicalCountries.map((c) => `<span style="color: #0f172a; font-weight: 500;">${escapeHtml(c.name)}</span>: ${c.users} users`))}
    </div>
    ` : ''}
...
function buildReportText(m: Metrics): string {
  return [
    ...
    '',
    ...(m.historicalCountries.length ? ['Historical top countries (All time):', ...m.historicalCountries.map((c) => `  - ${c.name}: ${c.users} users`), ''] : []),
    '',
